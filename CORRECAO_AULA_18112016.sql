...
select  v.codigo Código, 
        date_format(v.data_hora, "%d/%m/%Y") Data, 
        date_format(v.data_hora, "%T") Hora, 
        p.descricao Produto, 
        l.nome Loja, 
        vd.nome Vendedor
from venda v
inner join Produto p on v.cod_produto = p.codigo
inner join loja l  on v.cod_loja = l.codigo
inner join vendedor vd on v.cod_vendedor = vd.codigo;        
